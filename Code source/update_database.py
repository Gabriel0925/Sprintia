from app_ressource import messagebox, sqlite3, table_bdd
...
def maj_de_la_BDD_de_Sprintia_3_1_vers_Sprintia3_2():
    ...
    try:
        ...
        curseur.execute("DROP TABLE Aide_bienvenue")
    except sqlite3.Error as e:
        pass
...
def création():
    # Identifiant
    curseur.execute(f"CREATE TABLE IF NOT EXISTS {table_bdd[0]} (id INTEGER PRIMARY KEY AUTOINCREMENT,username TEXT UNIQUE NOT NULL,password TEXT NOT NULL, sport TEXT, bio TEXT)")
        
    # Historique_activité (Libre, Course, Musculation, Football)
    curseur.execute(f"""CREATE TABLE IF NOT EXISTS {table_bdd[1]} (id_activité INTEGER PRIMARY KEY AUTOINCREMENT,account_id INTEGER,date_activité TEXT,sport TEXT,durée INTEGER,rpe INTEGER,charge INTEGER,        
                    distance NUMERIC,dénivelé INTEGER,type TEXT, 
                    vitesse_max NUMERIC, allure TEXT,
                    muscle_travaillé TEXT, répétitions INTEGER, série INTEGER, volume NUMERIC, équipement TEXT, lieu TEXT,
                    score TEXT, humeur TEXT, but INTEGER, passe_décisive INTEGER,
                    catégorie TEXT, FOREIGN KEY (account_id)REFERENCES Account (id));""")
        
    # Planification
    curseur.execute(f"CREATE TABLE IF NOT EXISTS {table_bdd[2]} (id INTEGER PRIMARY KEY AUTOINCREMENT,nom TEXT NOT NULL,date TEXT NOT NULL,sport TEXT NOT NULL,objectif TEXT NOT NULL, lieu TEXT,priorité TEXT,account_id INTEGER,FOREIGN KEY (account_id) REFERENCES Account(id))")
    curseur.execute(f"CREATE TABLE IF NOT EXISTS {table_bdd[3]} (id INTEGER PRIMARY KEY AUTOINCREMENT,sport TEXT NOT NULL,date TEXT NOT NULL,objectif TEXT NOT NULL,fréquence TEXT NOT NULL,niveau_début TEXT NOT NULL,niveau_fin TEXT,statut TEXT, account_id INTEGER,FOREIGN KEY (account_id) REFERENCES Account(id))")
    curseur.execute(f"CREATE TABLE IF NOT EXISTS {table_bdd[4]} (id INTEGER PRIMARY KEY AUTOINCREMENT,account_id INTEGER NOT NULL,type TEXT,FOREIGN KEY (account_id) REFERENCES Account(id))")
    
    # Transfert de données
    curseur.execute(f"CREATE TABLE IF NOT EXISTS {table_bdd[5]} (action TEXT)")
    curseur.execute(f"CREATE TABLE IF NOT EXISTS {table_bdd[6]} (action TEXT)")

    # Connection automatique
    curseur.execute(f"CREATE TABLE IF NOT EXISTS {table_bdd[7]} (statut TEXT)")

    # Personalisation Coach
    curseur.execute(f"CREATE TABLE IF NOT EXISTS {table_bdd[8]} (account_id INTEGER NOT NULL, nom_du_coach TEXT, style_du_coach TEXT, avatar TEXT)")
