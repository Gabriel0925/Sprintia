...
def coach_pour_ajouter_une_activité(boite, account_id, app, exercice, date_entry, duree_entry, rpe_entry, douleur_entry, fatigue_entry, 
...
def vérification_data_de_base_activité(account_id, app, exercice, date_entry, duree_entry, rpe_entry, douleur_entry, fatigue_entry, 
    ...
    if mode == "Libre" or mode == "Course":
        denivele = None        
        try:
            deniv_str = denivele_entry.get().strip().replace(',', '.').replace(' ', '').replace('_', '').replace('m', '').replace('M', '')
            if not deniv_str:
                distance = None
            if deniv_str:
    ...
    if mode == "Musculation":
        ...
        if not humeur:
            ...
            return
        if len(score) > 20:
            messagebox.showerror("Erreur", "Le score ne doit pas dépasser 20 caractères !")
            return
        if score:
            pass
        else:
...
def supprimer_activité(account_id, période_str, app, sidebar_exercice, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    choix_entry = ctk.CTkEntry(master=carte_connexion, placeholder_text="ID de l'activité", border_color=couleur_fond, fg_color=couleur_fond,
    ...
    choix_entry.pack(expand=True, fill="both", padx=12, pady=(12, 2))
...
def ajouter_activité_course(account_id, app, sidebar_exercice, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    def mise_mode(choix):
        ...
        if choix == "Libre":
            app.after(0, lambda: [vider_fenetre(app), ajouter_activité_libre(account_id, app, sidebar_exercice, exercice, charge_entraînement, predicteur_temps, parametre)])
...
def ajouter_activité_musculation(account_id, app, sidebar_exercice, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    Options_douleur = {"Non": 0, "Un peu": 1, "Oui": 2, "Blessure": 3}
    Options_fatigue = {"Non": 0, "Un peu": 2.5, "Oui": 5}
    Options_lieu = {"Salle de sport": "Salle de sport", "Domicile": "Domicile", "Autre": "Autre"}
...
def ajouter_activité_libre(account_id, app, sidebar_exercice, exercice, charge_entraînement, predicteur_temps, parametre):
    Options_douleur = {"Non": 0, "Un peu": 1, "Oui": 2, "Blessure": 3}
    Options_climat = {"Normal": 0, "Chaud": 1, "Froid": 2, "Difficile": 3}
    Options_fatigue = {"Non": 0, "Un peu": 2.5, "Oui": 5}
    options_type = ["Intense", "Modéré", "Récupération", "Sport d'équipe", "Fun", "Brûle Graisse", "Tabata", "Sortie longue", "Expédition", "Cardio", "Autre"]
...
def interface_exercice(account_id, type_de_catégorie, headers, requête_sql, app, sidebar_exercice, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    button_creer_activite = ctk.CTkButton(master=boite_semi_header3, text="➕ Ajouter", fg_color=couleur2, hover_color=couleur2_hover,
                           corner_radius=corner1, height=button_height, font=(font_principale, taille3), text_color=couleur1,
                                        command=lambda: [vider_fenetre(app), ajouter_activité_libre(account_id, app, sidebar_exercice, exercice, charge_entraînement, predicteur_temps, parametre)])
...
def exercice_musculation(account_id, app, sidebar_exercice, exercice, charge_entraînement, predicteur_temps, parametre):
    requête_sql = "SELECT date_activité, lieu, durée, rpe, équipement, muscle_travaillé, répétitions, série, volume FROM Historique_activité WHERE account_id = ? AND catégorie = ? AND date_activité >= ? ORDER BY date_activité DESC"
