...
def ajouter_compétition(account_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    frame.pack(pady=10)
    Titre = ctk.CTkLabel(frame ,text="Ajouter une compétition", font=(font_secondaire, taille1))
...
def supprimer_compétition(account_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    Titre = ctk.CTkLabel(app ,text="Supprimer une compétition", font=(font_secondaire, taille1))
    ...
    try:
        ...
        if result:
            ...
            pas_données = ctk.CTkLabel(tableau_frame, text="Aucun objectif futur n'a été enregistré.", font=(font_principale, taille2))
...
def modifier_compétition_étape2(account_id, result_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):    
    ...
    frame = ctk.CTkFrame(app, fg_color="transparent")
    frame.pack(pady=10)
    Titre = ctk.CTkLabel(frame ,text="Modifier une compétition", font=(font_secondaire, taille1))
    Titre.pack(padx=10, pady=0)
...
def modifier_compétition_étape1(account_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    Titre = ctk.CTkLabel(app ,text="Modifier une compétition", font=(font_secondaire, taille1))
    Titre.pack(padx=10, pady=10)
    ...
    try:
        ...
        if result:
            ...
            pas_données = ctk.CTkLabel(tableau_frame, text="Aucune compétition future n'a été enregistré", font=(font_principale, taille1))
            pas_données.grid(row=1, column=0, columnspan=len(headers), pady=20)
    ...
    def maj_pour_tableau(choix):
        ...
        try:
            ...
            if objectif_result:
                ...
                pas_données = ctk.CTkLabel(tableau_frame, text="Aucune compétition future n'a été enregistrée.", font=(font_principale, taille1))
                pas_données.grid(row=1, column=0, columnspan=len(headers), pady=20)
        except sqlite3.Error as e:
            messagebox.showerror("Erreur", "Erreur de base de données lors de la récupération de tes compétitions !")
            return
...
def ajouter_objectif(account_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    frame = ctk.CTkFrame(app, fg_color="transparent")
    frame.pack(pady=10)
    Titre = ctk.CTkLabel(frame ,text="Ajouter un objectif", font=(font_secondaire, taille1))
    Titre.pack(padx=10, pady=0)
...
def modifier_objectif_étape2(account_id, result_id, app,sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):    
    ...
    frame = ctk.CTkFrame(app, fg_color="transparent")
    frame.pack(pady=10)
    Titre = ctk.CTkLabel(frame ,text="Modifier un objectif", font=(font_secondaire, taille1))
    Titre.pack(padx=10, pady=0)
...
def modifier_objectif_étape1(account_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    Titre = ctk.CTkLabel(app ,text="Modifier un objectif", font=(font_secondaire, taille1))
    Titre.pack(padx=10, pady=10)
    ...
    try:
        ...
        if result:
            ...
            pas_données = ctk.CTkLabel(tableau_frame, text="Aucun objectif futur n'a été enregistré", font=(font_principale, taille1))
            pas_données.grid(row=1, column=0, columnspan=len(headers), pady=20)
    except sqlite3.Error as e:
...
def supprimer_objectif(account_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    Titre = ctk.CTkLabel(app ,text="Supprimer un objectif", font=(font_secondaire, taille1))
    Titre.pack(padx=10, pady=10)
...
def mettre_en_pause_les_analyses(account_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre, mode):
    ...
    if result == "vacances":
        info_statut_actif = ctk.CTkLabel(frame, text=f"Ton statut d'entraînement actuel: Vacances", font=(font_principale, taille2))
        info_statut_actif.pack(padx=0, pady=10)
    elif result == "blessure":
        info_statut_actif = ctk.CTkLabel(frame, text=f"Ton statut d'entraînement actuel : Blessure", font=(font_principale, taille2))
        info_statut_actif.pack(padx=0, pady=10)
    elif result == "suspendre":
        info_statut_actif = ctk.CTkLabel(frame, text=f"Ton statut d'entraînement actuel : Suspendre", font=(font_principale, taille2))
        info_statut_actif.pack(padx=0, pady=10)
    else:
        info_statut = ctk.CTkLabel(frame, text="Ton statut d'entraînement actuel : Actif", font=(font_principale, taille2))
...
def indulgence_de_course(account_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    info_nom_coach = ctk.CTkLabel(frame_jrm_coach, text="👨 JRM Coach", font=(font_secondaire, taille2), wraplength=280, text_color=couleur1,
                                  justify="left")
...
def charge_d_entraînement(account_id, app, sidebar_performance, exercice, charge_entraînement, predicteur_temps, parametre):
    ...
    graphique.pack(expand=True, fill="both", pady=(0, 5))
    ...
    info_nom_coach = ctk.CTkLabel(interprétation_conseil, text="👨 JRM Coach", font=(font_secondaire, taille2), wraplength=280, text_color=couleur1,
                                  justify="left")
    ...
    if pause == "blessure":
        ...
        if ratio is not None:
            if ratio < 0.5: 
                catégorie_statut = ctk.CTkLabel(h1_result_optimale, text="🛌 Récupération active", font=(font_secondaire, taille2),
                                                width=300, wraplength=280, text_color=couleur1)
                catégorie_statut.pack(fill="both", expand=True, padx=12, pady=12)             
                interpretation_statut = ctk.CTkLabel(interprétation_conseil, text="Charge très basse. Tu laisse ton corps se reposer mais jusqu'à quand !\n\n" \
                ...
                catégorie_statut = ctk.CTkLabel(h1_result_optimale, text="🔄 Maintien", font=(font_secondaire, taille2),
                                                width=300, wraplength=280, text_color=couleur1)
                catégorie_statut.pack(fill="both", expand=True, padx=12, pady=12)             
                interpretation_statut = ctk.CTkLabel(interprétation_conseil, text="Charge adaptée pour conserver ton niveau\n\n" \
                ...
                catégorie_statut = ctk.CTkLabel(h1_result_optimale, text="💪 Progression élévée", font=(font_secondaire, taille2),
                                                width=300, wraplength=280, text_color=couleur1)
                catégorie_statut.pack(fill="both", expand=True, padx=12, pady=12)             
                interpretation_statut = ctk.CTkLabel(interprétation_conseil, text="Tu progresse vite mais fait attention aux blessures\n\n" \
                "Surveille bien la fatigue de ton corps pour éviter les blessures : en gardant cette charge quelques semaines, le risque reste limité.",
    ...
    try:   
        if data_pour_graphique:
            ...
            pas_de_données = ctk.CTkLabel(not_data, text="L'algorythme ne peut pas te fournir des conseils, des données parce que tu n'as pas assez de données pour afficher un graphique.\n\nAjoute quelques séances d'entraînement pour voir l'évolution de ta charge d'entraînement au cours du temps.",
                                          font=(font_secondaire, taille2), text_color=couleur1, wraplength=600, justify="left")
            pas_de_données.pack(padx=15, pady=15)
            button_creer_activite = ctk.CTkButton(not_data, text="ℹ️  Aide", fg_color=couleur2, hover_color=couleur2_hover,
